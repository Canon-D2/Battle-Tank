...
def update():
    ...
    # Nếu đá chạm vào rìa thì random lại
    if rock.y > 600:
        rock.y = 0
        rock.x = random.randint(20,780)

    # Vật phẩm và đá không được random cùng 1 chỗ
...
def draw():
    background.draw()
    if ket_thuc:
        screen.draw.text('Tro choi ket thuc',(210,200), color = WHITE, fontsize=60)
        screen.draw.text('Tong diem: '+ str(diem),(210,250), color = VIOLET, fontsize=80)
        screen.draw.text('(Nhan SPACE de choi lai)',(210,330), color = BLUE, fontsize=40)
        if keyboard.SPACE:
            reset_game()
    else:
